...
class AttendanceManager(tk.Tk):
    def _init_(self, *args, **kwargs):
        ...
        self.subject_queue = deque()  # Initialize a deque for storing subjects
...
class ManageAttendance(tk.Frame):
    def _init_(self, parent, controller):
        ...
        bt1 = tk.Button(self, text="Back to home", bg="red", font=("Times", 16), height=2, width=17,
                        command=lambda: controller.show_frame(StartPage))
        bt4 = tk.Button(self, text="Add to queue", bg="purple", font=("Times", 16), height=2, width=17,
                        command=lambda: self.add_to_queue(controller))
        bt5 = tk.Button(self, text="Show queue", bg="blue", font=("Times", 16), height=2, width=17,
                        command=lambda: self.show_queue(controller))
        bt2.pack()
        bt3.pack()
        bt1.pack()
        bt4.pack()
    ...
    def add_to_queue(self, controller):
        try:
            conn = sql.connect("attend")
            cur = conn.cursor()
            cur.execute('SELECT * FROM attable')
            for w in cur:
                controller.subject_queue.append(w[1])  # Add each subject to the queue
            messagebox.showinfo("Queue", "Subjects added to the queue.")
            conn.commit()
            conn.close()
        except:
            messagebox.showinfo("Alert!", "There is no record")

    def show_queue(self, controller):
        queue_text = ", ".join(controller.subject_queue)
        if queue_text:
            messagebox.showinfo("Queue", f"Subject Queue: {queue_text}")
        else:
            messagebox.showinfo("Queue", "Subject Queue is empty.")

    def sort_subjects(self):
        try:
            conn = sql.connect("attend")
            cur = conn.cursor()
            cur.execute('SELECT * FROM attable ORDER BY subject')
            text = ""
            for w in cur:
                text = text + "sub id " + str(w[0]) + " " + w[1] + "\n"
            messagebox.showinfo("Sorted Subjects", text)
            conn.commit()
            conn.close()
        except:
            messagebox.showinfo("Alert!", "There is no record")
